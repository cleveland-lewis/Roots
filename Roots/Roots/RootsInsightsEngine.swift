...
struct TimeEstimationModel {
    func predictedTotalTime(for assignment: AssignmentSnapshot, history: [AssignmentMetrics]) -> TimeInterval {
        ...
        if let est = assignment.estimatedMinutes { baseMinutes = Double(est) }
            ...
            // light defaults by broad category guess
            if let category = assignment.category?.lowercased(), category.contains("read") {
                baseMinutes = 90
            } else if let category = assignment.category?.lowercased(), category.contains("exam") {
                baseMinutes = 180
            } else {
                baseMinutes = 120
            }
